...
export class TranchesDeployments {
  ...
  constructor(params: {
    ...
    this.ds = new Deployments(params.client, params.deployer, {
      ...
      whenBytecodeChanged:
        params.deployments ?? (this.isTestnet() ? null : "throw"),
      fork: params.client.forked?.platform,
    });

    let info = JSON.parse(
      JSON.stringify(Tranches.ethena)
    ) as typeof Tranches.ethena;

    if (this.platform.Tranches?.ethena) {
      info.jrt = {
        ...info.jrt,
        ...(this.platform.Tranches.ethena.jrt ?? {}),
      } as any;
      info.srt = {
        ...info.srt,
        ...(this.platform.Tranches.ethena.srt ?? {}),
      } as any;
    }
    this.ethenaInfo = info;
  }

  async get<T extends ContractBase>(
    Ctor: Constructor<T>,
    params?: { id?: "jrUSDe" | "srUSDe"; cdo?: "USDe" }
  ) {
    if (params?.id === "jrUSDe") {
    ...
    let byCdo = byName.filter((x) =>
      x.id.toLowerCase().includes(cdo.toLowerCase())
    );
    if (byCdo.length === 1) {
    ...
    if (byCdo.length > 1) {
      throw new Error(
        `Multiple ${Ctor.name} found for CDO ${cdo}: ${byCdo
          .map((x) => x.id)
          .join(", ")}`
      );
    }
  ...
  async ensureEthena() {
    ...
    if (network === "hardhat") {
      ...
      let sUSDe = await this.ds.ensureContract(MockStakedUSDe, {
        arguments: [USDe.address, this.owner.address, this.owner.address],
      });
      ...
      await sUSDe
        ...
        .setCooldownDuration(
          this.owner,
          $date.parseTimespan("1week", { get: "s" })
        );
    ...
    if (network === "hoodi") {
      ...
      let sUSDeAddress = $require.Address(
        this.platform.Tokens["sUSDe"].address
      );
      let pUSDeAddress = $require.Address(
        this.platform.Tokens["pUSDe"].address
      );
      let sUSDS = await this.ds.ensureContract(MockStakedUSDS, {
        arguments: [USDeAddress],
      });
      return {
  ...
  async ensureACM() {
    ...
    if (this.isTestnet() === false) {
      ...
      await this.ds.configure(acm, {
        ...
        shouldUpdate:
          deployerIsAdmin &&
          $address.eq(deployer.address, owner.address) === false,
        async updater() {
          await acm.$receipt().revokeRole(owner, "0x", deployer.address);
        },
      });
  ...
  async ensureConfigManager() {
    ...
    let { contract: configManager } = await this.ds.ensureWithProxy(
    ...
    await this.ds.configure(cdo, {
      ...
      async shouldUpdate() {
        return (
          $address.eq(
            await cdo.twoStepConfigManager(),
      ...
      async updater() {
        await cdo
          .$receipt()
          .setTwoStepConfigManager(owner, configManager.address);
      },
    });
  ...
  async ensureCooldowns() {
    const acm = await this.ensureACM();
    const { contract: erc20Cooldown } = await this.ds.ensureWithProxy(
      ERC20Cooldown,
      {
        initialize: [this.owner.address, acm.address],
      }
    );
    const { contract: unstakeCooldown } = await this.ds.ensureWithProxy(
      UnstakeCooldown,
      {
        initialize: [this.owner.address, acm.address],
      }
    ...
    const { contractBeaconProxy: sUSDeCooldownRequestImpl } =
      await this.ds.ensureWithBeacon(SUSDeCooldownRequestImpl, {
    ...
    await this.ds.configure(unstakeCooldown, {
      ...
      updater: async () => {
        await unstakeCooldown
          .$receipt()
          .setImplementations(
            this.owner,
            [sUSDe.address],
            [sUSDeCooldownRequestImpl.address]
          );
      },
    });
  ...
  async ensureFeeds() {
    ...
    const { contract: sUSDeAprPairProvider } = await this.ds.ensure(
      SUSDeAprPairProvider,
      {
        arguments: [sUSDS.address, sUSDe.address],
      }
    );
    ...
    const stalePeriodAfter = $date.parseTimespan(
      this.platform.Feed.stalePeriodAfter,
      { get: "s" }
    );
  ...
  async ensureEthenaCDO() {
    ...
    await this.ensureRole(
      await erc20Cooldown.COOLDOWN_WORKER_ROLE(),
      strategy.address
    );
    ...
    await this.ds.configure(cdo, {
      shouldUpdate: async () =>
        $address.eq(await cdo.strategy(), $address.ZERO),
  ...
  async configure(
    ...
    await this.ensureRole(
      $contract.keccak256("UPDATER_CDO_APR_ROLE"),
      feed.address
    );
  ...
  async setTrancheActions(
    ...
    let actions =
      type === "jrt" ? await cdo.actionsJrt() : await cdo.actionsSrt();

    let current = type === "jrt" ? info.jrt : info.srt;

    if (
      actions.isDepositEnabled !== current.depositsEnabled ||
      actions.isWithdrawEnabled !== current.withdrawalsEnabled
    ) {
      await cdo
        .$receipt()
        .setActionStates(
  ...
  async setCooldown(
    ...
    await this.ds.configure(strategy, {
      ...
      updater: async () => {
        await strategy
          .$receipt()
          .setCooldowns(this.owner, cooldowns[0], cooldowns[1]);
      },
    });
  }

  private async initialDeposit(tranches: {
    jrtVault: Tranche;
    srtVault: Tranche;
    cdo: StrataCDO;
  }) {
    ...
    if (balance < AMOUNT) {
      if (
        this.client.network === "hardhat" ||
        this.client.network === "hoodi"
      ) {
        await USDe.$receipt().mint(
          this.owner,
          this.owner.address,
          AMOUNT * 100n
        );
    ...
    await cdo
      .$receipt()
      .setActionStates(this.owner, $address.ZERO, true, false);
    await $erc4626.deposit(jrtVault as any, this.owner, AMOUNT / 2n);
    await $erc4626.deposit(srtVault as any, this.owner, AMOUNT / 2n);
    await cdo
      .$receipt()
      .setActionStates(this.owner, $address.ZERO, false, false);
  }
  private async initialDepositAtomic(tranches: {
    jrtVault: Tranche;
    srtVault: Tranche;
    cdo: StrataCDO;
  }) {
    ...
    let actions = [
      ...
      await jrtVault
        .$data()
        .deposit(this.owner, AMOUNT_JRT, this.owner.address),

      await USDe.$data().approve(this.owner, srtVault.address, AMOUNT_SRT),
      await srtVault
        .$data()
        .deposit(this.owner, AMOUNT_SRT, this.owner.address),
    ...
    let tx = await safeTx.executeBatch(...actions);
    await tx.wait();
  }
  ...
  async ensureDepositor() {
    ...
    let { contract: depositor } = await this.ds.ensureWithProxy(
      TrancheDepositor,
      {
    ...
    await this.ensureRole(
      $contract.keccak256("DEPOSITOR_CONFIG_ROLE"),
      this.owner.address
    );
    ...
    if (pUSDe) {
      ...
      if (status === false) {
        await depositor
          .$receipt()
          .addAutoWithdrawals(this.owner, [pUSDe.address], [true]);
      }
    }
    return depositor;
  }
